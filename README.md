...
Follow the instructions in [pokemon-showdown/server/README](pokemon-showdown/server/README) to start the Showdown server. It should be running on `localhost:8000`.

#### Foul play set Set-up
```
cd foul-play
pip install -r requirements.txt # I recommend doing this in a Python venv
make poke_engine_local GEN=gen6
```

#### Running foul-play

Open the Showdown server on `localhost:8000`. In the top right select `Choose name` and create a name for your bot to play under (e.g. `bot_1`), do not enter a password. If the name is taken you will need to come up with another. Log out and create another player just like you did for `bot_1` (e.g. create `bot_2`). This step registers two accounts which can now challenge each other. 

The following command will connect `foul-play`'s bot to the Pokemon Showdown server on `localhost:8000`, log it in with the name `498_bot_1`, challenge the user with the name `498_bot_2` to a gen6 random battle using the UCB exploration method. 
```
python run.py \
    --websocket-uri ws://localhost:8000/showdown/websocket \ # assuming ur running on localhost:8000
    --ps-username 'bot_1' \
    --bot-mode challenge_user \
    --user-to-challenge 'bot_2' \
...
Ensure you run this command to log `bot_2` in, BEFORE you run the command for `bot_1`.
